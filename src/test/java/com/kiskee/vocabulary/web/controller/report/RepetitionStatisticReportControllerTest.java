...
@WebMvcTest(RepetitionStatisticReportController.class)
public class RepetitionStatisticReportControllerTest {
    ...
    void testGetReport_WhenReportExists_ThenReturnReportDto() {
        ReportDto reportDto = new RepetitionStatisticReportDto(List.of(
                new BaseReportRowDto(LocalDate.of(2024, 7, 18), LocalDate.of(2024, 7, 18), 1, "daily", List.of())));

        when(repetitionStatisticReportService.getReport()).thenReturn(reportDto);

        MvcResult result = mockMvc.perform(get("/report/repetition-statistic"))
                .andDo(print())
    ...
    void testGetReport_WhenReportDoesNotExist_ThenReturnNotFoundStatus() {
        ...
        when(repetitionStatisticReportService.getReport())
                .thenThrow(new ResourceNotFoundException("There is no report yet"));

        mockMvc.perform(get("/report/repetition-statistic"))
                .andDo(print())
                .andExpectAll(
                        status().isNotFound(),
