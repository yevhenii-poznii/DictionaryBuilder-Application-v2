...
create table repetition_statistic_report
(
    id      bigserial not null primary key,
    user_id uuid      not null unique,
...
create table repetition_statistic_report_row
(
    id            bigserial   not null primary key,
    start_period  date        not null,
    end_period    date        not null,
    ...
    constraint fk_repetition_statistic_report_id foreign key (report_id) references repetition_statistic_report (id) on delete cascade
);

create table dictionary_repetition_statistic_report
(
    id                    bigserial        not null primary key,
    dictionary_id         bigint           not null,
    dictionary_name       varchar(50)      not null,
    ...
    wrong_answers_rate    double precision not null,
    wrong_answers_count   integer          not null,
    skipped_words_rate    double precision not null,
    skipped_words_count   integer          not null,
    total_words_passed    integer          not null,
    completed_repetitions integer          not null,

    report_row_id         bigint,

    constraint fk_repetition_statistic_report_row_id foreign key (report_row_id) references repetition_statistic_report_row (id) on delete cascade
);

create index idx_repetition_statistic_report_user_id on repetition_statistic_report (user_id);
create index idx_repetition_statistic_report_row_report_id on repetition_statistic_report_row (report_id);
create index idx_dictionary_repetition_statistic_report_report_row_id on dictionary_repetition_statistic_report (report_row_id);
